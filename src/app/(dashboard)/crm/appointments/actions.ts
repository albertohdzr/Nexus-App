...
export async function saveAppointmentSettings(
  ...
  const allowOverbooking =
    (formData.get("allow_overbooking") as string | null) === "on";
...
export async function generateSlots(
  ...
  const startMinutes = parseTimeToMinutes(
    effectiveSettings.start_time || "08:00",
  );
  ...
  (blackouts || []).forEach((b) => {
    ...
    blackoutMap.set(b.date, [...(blackoutMap.get(b.date) || []), {
      start,
      end,
    }]);
  });
    ...
    if (allowedDays.includes(dayOfWeek)) {
      ...
      while (currentMinute < endMinutes) {
        ...
        if (!overlaps) {
          ...
          const paddedStart = `${String(slotStartHour).padStart(2, "0")}:${
            String(slotStartMin).padStart(2, "0")
          }:00`;
          const paddedEnd = `${String(slotEndHour).padStart(2, "0")}:${
            String(slotEndMin).padStart(2, "0")
          }:00`;

          const startAbsolute = fromZonedTime(
            `${currentStr} ${paddedStart}`,
            timeZone,
          );
          const endAbsolute = fromZonedTime(
            `${currentStr} ${paddedEnd}`,
            timeZone,
          );
  ...
  revalidatePath("/crm/appointments");
  return {
    success: "Disponibilidad generada",
    inserted: count || slotsToInsert.length,
  };
}
...
export async function blockDay(
  ...
  if (error) {
    ...
    return {
      error: "Se registró el bloqueo pero no se pudieron actualizar los slots",
    };
  }
